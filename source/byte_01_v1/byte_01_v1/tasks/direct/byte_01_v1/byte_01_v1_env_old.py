...
class Byte01V1Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Byte01V1EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._die_body_ids, _ = self._contact_sensor.find_bodies(
              ["base_link", "hip1", "hip2", "hip3", "hip4"]   # ← terminate if any of these touch ground
              )
        self._feet_ids, _     = self._contact_sensor.find_bodies(
            ["feet", "feet2", "feet3", "feet4"]              # ← new fixed-joint foot tip links
            )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        died = torch.any(
            torch.max(
                torch.norm(net_contact_forces[:, :, self._die_body_ids], dim=-1),dim=1,)[0] > 150.0,dim=1,)
